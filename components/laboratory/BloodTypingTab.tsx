...
const tests = ['ABO Typing', 'Rh Typing', 'Cross-matching']
const specimens = ['Whole Blood', 'Serum', 'EDTA Blood']
const allResults = ['A', 'B', 'AB', 'O', 'Rh Positive (+)', 'Rh Negative (-)', 'Compatible', 'Incompatible']

const NORMAL_VALUES = {
  result_date: format(new Date(), 'yyyy-MM-dd'),
  rows: [
    { test: 'ABO Typing', specimen: 'EDTA Blood', result: 'O' },
    { test: 'Rh Typing', specimen: 'EDTA Blood', result: 'Rh Positive (+)' },
  ],
  is_normal: true,
}
    ...
    <tr className="border-b border-[hsl(var(--border)/0.5)] hover:bg-[hsl(var(--accent)/0.3)] transition-colors">
      ...
      <td className="px-4 py-3">
        <Select value={row.result || ''} onValueChange={v => set('result', v)}>
          ...
          <SelectContent>{allResults.map(r => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent>
        </Select>
      </td>
    </tr>
  )
...
export default function BloodTypingTab({ data, onChange }: { data: any; onChange: (v: any) => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <SectionCard title="Blood Typing Results">
      ...
      <div className="flex items-center gap-2 pt-2">
        <Checkbox id="normal-bt" checked={!!data.is_normal} onCheckedChange={v => set('is_normal', v)} />
        <label htmlFor="normal-bt" className="text-sm font-semibold text-[hsl(var(--primary))] cursor-pointer px-2 py-1 rounded-lg hover:bg-[hsl(var(--accent)/0.5)] transition-colors">NORMAL</label>
      </div>
    </div>
  )
}
