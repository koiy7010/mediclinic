...
import { Save, Search, ScanLine } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[hsl(var(--background))] gap-4 px-4">
        <div className="w-14 h-14 rounded-full bg-[hsl(var(--primary)/0.1)] flex items-center justify-center">
          <ScanLine className="w-7 h-7 text-[hsl(var(--primary))]" />
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {['Yes', 'No'].map(opt => (
              <label key={opt} className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="utz_normal" value={opt} checked={form.is_normal === (opt === 'Yes')} onChange={() => set('is_normal', opt === 'Yes')} className="w-4 h-4 accent-[hsl(var(--primary))]" />
